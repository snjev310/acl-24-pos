...
# %%
count_1 = 0
for item in zip(bho_to_len,bho_tr_len):
    count_1 += 1
    if item[0]-item[1]==2:
        pass
    else:
...
hindi_token = [x for x in hindi_token if x != '<s>' and x!='</s>']

angika_token = sum(angika_token_new,[])
angika_token = [x for x in angika_token if x != '<s>' and x!='</s>']

magahi_token = sum(magahi_token_new,[])
magahi_token = [x for x in magahi_token if x != '<s>' and x!='</s>']

bhojpuri_token = sum(bhojpuri_token_new,[])
bhojpuri_token = [x for x in bhojpuri_token if x != '<s>' and x!='</s>']
