...
print(len(hindi_token)==len(hindi_true_label))
print(len(hindi_token)==len(hindi_predicted_label))
...
print(len(bhojpuri_token)==len(bhojpuri_true_label))
print(len(bhojpuri_token)==len(bhojpuri_predicted_label))
# %%
def compute_metrics(eval_preds):
    pred_logits,labels = eval_preds
...
maximum_ = []
for i in range(706):
    for index,item in enumerate(angika_eval_result_score['eval_labels'][i]):
        if item !=-100 :
            #print(angika_eval_result['eval_pred_logits'][i][index])
            lab = np.argmax(angika_eval_result_score['eval_pred_logits'][i][index])
            label_.append(lab)
            #print("label: ",lab)
            pred_mat = (angika_eval_result_score['eval_pred_logits'][i][index])
            #print("Maximum: ",maxi)
...
print(len(bhojpuri_pred_label_)==len(bhojpuri_pred_label_max_score))
...
def look_back(token,truth,predicted,label,max_list):
    ...
    for i in range(len(token)-2):
        if '##' not in token[i] and '##' in token[i+1] and '##' not in token[i+2]:
        ...
        elif '##' not in token[i] and '##' in token[i+1] and '##' in token[i+2] and '##' not in token[i+3]:
            ...
            #print("Swapped: ",swapped_predicted)
        elif '##' not in token[i] and '##' in token[i+1] and '##' in token[i+2] and '##' in token[i+3] and '##' not in token[i+4]:
            ...
            #print("Swapped: ",swapped_predicted)
        elif '##' not in token[i] and '##' in token[i+1] and '##' in token[i+2] and '##' in token[i+3] and '##' in token[i+4] and '##' not in token[i+5]:
...
# %%
hi_swapped_pred = look_back(hi_token,hi_truth,hi_predicted,hi_label,hi_max_list)

ang_swapped_pred = look_back(ang_token,ang_truth,ang_predicted,ang_label,ang_max_list)

mag_swapped_pred = look_back(mag_token,mag_truth,mag_predicted,mag_label,mag_max_list)

bho_swapped_pred = look_back(bho_token,bho_truth,bho_predicted,bho_label,bho_max_list)
# %%
